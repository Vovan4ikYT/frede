...
canvas.create_text(960, 35, text='ВНИМАНИЕ!\nДанную инструкцию '
                                 ...
                                 'тем, \nкто впервые в Fazbear Entertainment.\nP.S. '
                                 'Если хотите посмотреть концовку, '
                                 'нужно запускать файл main_launch.py', fill='#842593')
...
def open_gameplay():
    ...
    canvas1.create_text(960, 25, text='Теперь прочитайте это сообщение '
                                      ...
                                      'не выберетесь отсюда.', fill='#842593')
    canvas1.create_text(960, 75, text='Ещё не стоит забывать, \n'
                                      ...
                                      'но их будет видно.', fill='#842593')
    canvas1.create_text(960, 125, text='Ну и наконец, всё здание питается от электричества.\n'
                                       'Открытие камеры тратит 1 процент заряда, '
                                       'закрытие двери - 2 процента.\nЕсли энергия '
                                       'кончится, то это конец!', fill='#842593')
    ...
    def sister_cam():
        ...
        count_sc += 1
        if count_sc % 2 != 0:
            sis_cam = canvas1.create_text(960, 200, text='Почти у каждого аниматроника '
                                               'есть своя так называемая '
                                               '"родственная камера": если аниматроник стоит у офиса, '
                                               ...
                                               'становятся для него родственными.\nТо же самое и с Фредди', fill='#842593')
        else:
...
def open_anim():
    ...
    canvas2.create_text(960, 25, text='Добро пожаловать на работу '
                                      'в Fazbear Entertainment. \nПишет Уильям Афтон - '
                                      'создатель всех аниматроников. \nЗдесь я подробно расскажу, '
                                      'как найти подход к каждому из них.', fill='#842593')
    ...
    def freddy():
        ...
        count_fred += 1
        if count_fred % 2 != 0:
            canvas2.create_image(1400, 470, image=anims[0], anchor='nw', tag='freddy')
            fred = canvas2.create_text(1530, 400, text='Как же я обожаю '
                                                       ...
                                                       'лучше уступить дорогу Фредди Фазберу...', fill='white')
        else:
            canvas2.delete(fred)
    ...
    def bonnie():
        ...
        count_bonn += 1
        if count_bonn % 2 != 0:
            canvas2.create_image(250, 470, image=anims[1], anchor='nw', tag='bonnie')
            bonn = canvas2.create_text(340, 400, text='Кролик Бонни - '
                                                       'местный гитарист, '
    ...
    def chica():
        ...
        count_chic += 1
        if count_chic % 2 != 0:
            canvas2.create_image(760, 470, image=anims[2], anchor='nw', tag='chica')
            chic = canvas2.create_text(860, 400, text='Цыплёнок Чика любит есть - такова её программа.\n'
                                                      ...
                                                      'закрой дверь.\nДа, кстати, '
                                                      'Кекс - тоже аниматроник.', fill='white')
        else:
            canvas2.delete(chic)
    ...
    def foxy():
        ...
        count_fox += 1
        if count_fox % 2 != 0:
            canvas2.create_image(960, 796, image=anims[-1], anchor='nw', tag='foxy')
            fox = canvas2.create_text(850, 796, text='Этот товарищ всегда был немного дёрганым.\n'
                                                     ...
                                                     'и он не будет двигаться.', fill='white')
        else:
            canvas2.delete(fox)
            canvas2.delete('foxy')
